...
if (!empty($_POST)){ // si le formulaire est posté
        ...
        if(strlen($_POST['nom']) < 2 || strlen($_POST['nom']) > 20){
            $contenu .= '<div class="bg-danger">Le nom de passe doit contenir au moins 2 caractères</div>';
        }

        if(strlen($_POST['prenom']) < 2 || strlen($_POST['prenom']) > 20){
            $contenu .= '<div class="bg-danger">Le prenom de passe doit contenir au moins 2 caractères</div>';
        }
        ...
        // Validation du code postal avec expression régulière:
        if(!preg_match('#^[0-9]{5}$#', $_POST['code_postal'])){ // preg_match retourne true si le string en deuxieme argument correspond à l'expression régulière'
    ...
    if(empty($contenu)){ // si $contenu est vide, c'est qu'il n'y a pas d'erreur
        ...
        if($membre->rowCount() > 0){
            ...
            //si le pseudo est unique on peut faire une inscription en BDD: 

            $_POST['mdp'] = md5($_POST['mdp']); // permet d'encrypter le mot de passe selon l'algorithme md5. Il faudra le faire aussi sur la page de connexion pour comparer 2 mots cryptés.$_COOKIE
            ...
            $contenu .= '<div class="bg-success">Vous êtes inscrit. <a href="connexion.php">Cliquer ici pour vous connecter</a></div>';
...
echo $contenu; // affiche les messages du similar_text
...
endif; // syntaxe du if avec deux points qui remplacent la première accollade et endif qui remplace la seconde
