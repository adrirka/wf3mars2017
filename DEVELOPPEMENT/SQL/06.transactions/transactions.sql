...
-- Une transaction permet de lancer des requêtes, telles que des modifications, et de les annuler si besoin, comme si vous pouviez faire un 'CTRL+Z' dans votre BDD

-- Transaction simple:
START TRANSACTION; -- démarre la transaction 
    SELECT * FROM employes; -- pour voir la table avant modifications
    UPDATE employes SET prenom = 'ALLO' WHERE id_employes = 739;

ROLLBACK; -- donne l'ordre à MySQL d'annuler la transaction, l'employe reprenant son prénom
